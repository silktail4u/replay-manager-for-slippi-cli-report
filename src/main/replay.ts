...
export async function getReplayFromLink(
  url: string,
): Promise<{ replays: Replay[]; invalidReplays: InvalidReplay[] }> {
  const replays: Replay[] = [];
  const invalidReplays: InvalidReplay[] = [];
  ...
  try {
    ...
    // Optional: write to temp file if needed for your parser

    try {
      const replay = await parseReplay(buffer,url); // You should already have this function
      replays.push(replay);
    } catch (err) {
      invalidReplays.push({ file: url, error: (err as Error).message });
    } finally {
...
export async function parseReplay(buffer:any,link:string) {
    async (buffer: any): Promise<Replay | InvalidReplay> => {
...
export async function getReplaysInDir(
  ...
  const objs = (await readdir(dir, { withFileTypes: true }))
    .filter((dirent: { isFile: () => any; name: string; }) => dirent.isFile() && dirent.name.endsWith('.slp'))
    .map(async (dirent: { name: any; }): Promise<Replay | InvalidReplay> => {
  ...
  const replays = (
    (await Promise.all(objs)).filter(
      (replayOrInvalidReplay: InvalidReplay) =>
    ...
    try {
      ...
      if (writeDisplayNames) {
        ...
        replay.players.forEach((player, j) => {
          ...
          newDisplayNameBuffer.forEach((byte: number) => {
